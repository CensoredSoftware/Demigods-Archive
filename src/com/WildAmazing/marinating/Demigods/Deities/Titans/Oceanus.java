...
public class Oceanus implements Deity
{
	...
	public void printInfo(Player p)
	{
		if(DUtil.isFullParticipant(p) && DUtil.hasDeity(p, getName()))
		{
			...
			if(((Oceanus) DUtil.getDeity(p, getName())).SKILLBIND != null) p.sendMessage(ChatColor.AQUA + "    Bound to " + ((Oceanus) DUtil.getDeity(p, getName())).SKILLBIND.name());
	...
	private boolean squidfire(Player p)
	{
		...
		if(!DUtil.canLocationPVP(target)) return false;

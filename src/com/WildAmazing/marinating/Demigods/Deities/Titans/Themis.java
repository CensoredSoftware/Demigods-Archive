...
public class Themis implements Deity
{
	...
	public void printInfo(Player p)
	{
		if(DMiscUtil.isFullParticipant(p) && DMiscUtil.hasDeity(p, getName()))
		{
			...
			if(((Themis) DMiscUtil.getDeity(p, getName())).SKILLBIND != null) p.sendMessage(ChatColor.AQUA + "    Bound to " + ((Themis) DMiscUtil.getDeity(p, getName())).SKILLBIND.name());
		...
		if(DMiscUtil.hasDeity(p, getName()))
		{
			...
			else if(str.equalsIgnoreCase(ult))
			{
				...
				if(DMiscUtil.getFavor(p) >= ULTIMATECOST)
				{
					...
					{
						if(DMiscUtil.isFullParticipant(pl) && DMiscUtil.getActiveEffectsList(pl.getName()).contains("Congregate"))
						{
	...
	{
		Player p = DMiscUtil.getOnlinePlayer(getPlayerName());
		DMiscUtil.addActiveEffect(p.getName(), "Congregate Call", 60);
		int count = 0;
		for(Player pl : p.getWorld().getPlayers())
		{
			...
			{
				count++;
				if(!p.equals(pl) && !DMiscUtil.getActiveEffectsList(pl.getName()).contains("Congregate"))
				{
					...
					DMiscUtil.addActiveEffect(pl.getName(), "Congregate", 60);
				}
			}
		}
		return count;

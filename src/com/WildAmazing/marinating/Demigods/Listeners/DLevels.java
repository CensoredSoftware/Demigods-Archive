...
public class DLevels implements Listener
{
	...
	public void gainEXP(BlockBreakEvent e)
	{
		if(e.getPlayer() != null)
		{
			...
			try
			{
				if(!DUtil.canWorldGuardBuild(p, e.getBlock().getLocation())) return;
			}
			catch(Exception ex)
			{
				// Do nothing
	...
	public void gainEXP(EntityDamageByEntityEvent e)
	{
		if(e.getDamager() instanceof Player)
		{
			...
			try
			{
				if(!DUtil.canWorldGuardBuild(p, e.getEntity().getLocation())) return;
			}
			catch(Exception ex)
			{
				// Do nothing
	...
	{
		if(DUtil.isFullParticipant(p)) if(DUtil.getAscensions(p) >= DUtil.ASCENSIONCAP) return;

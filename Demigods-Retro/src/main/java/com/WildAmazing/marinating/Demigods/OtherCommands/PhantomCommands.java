            ...
            public void run()
            {
                ...
                for(Player online : Bukkit.getOnlinePlayers())
                {
                    for(OfflinePlayer player : phantomized)
                        if(online.canSee(player.getPlayer()) && !online.equals(player)) online.hidePlayer(player.getPlayer());

                    if((plugin.isTitan(online) || plugin.isGod(online)) && !plugin.getInfo(online).isAlive()) phantomize(online);
                }

		...
		else if(ee instanceof EntityDeathEvent)
		{
			...
			if(e.getEntity() instanceof Player)
			{
				...
				if(dc == DamageCause.ENTITY_ATTACK || dc == DamageCause.SUICIDE)
				{
					...
					if(DMiscUtil.canWorldGuardPVP(p.getLocation()) && (DMiscUtil.getNearbyShrine(e.getEntity().getLocation()) == null || !DMiscUtil.getDeityAtShrine(DMiscUtil.getNearbyShrine(e.getEntity().getLocation())).equals("Typhon"))) p.getWorld().createExplosion(p.getLocation(), EXPLOSIONSIZE);
				}
			}
		}

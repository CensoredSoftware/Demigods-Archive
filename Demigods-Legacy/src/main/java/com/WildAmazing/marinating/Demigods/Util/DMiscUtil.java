...
{
	private static final Demigods plugin = (Demigods) Bukkit.getServer().getPluginManager().getPlugin("Demigods-Legacy"); // obviously needed
	 ...
	 * @return
	 */
	public static boolean hasPermissionOrOP(Player p)
	...
	public static void giveDeity(String p, Deity d)
	{
		...
		if(BROADCASTNEWDEITY) plugin.getServer().broadcastMessage(ChatColor.YELLOW + p + " has joined the lineage of " + d.getName() + ".");
	...
	public static boolean hasAdvantage(String alliance)
	{
		...
		for(String player : DSave.getCompleteData().keySet())
		{
			...
			{
				if(DSave.hasData(player, "LASTLOGINTIME")) if((Long) DSave.getData(player, "LASTLOGINTIME") < System.currentTimeMillis() - 604800000) continue;

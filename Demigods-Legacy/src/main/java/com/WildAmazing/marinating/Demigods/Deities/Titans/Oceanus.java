...
public class Oceanus implements Deity {
    ...
    public void printInfo(Player p) {
        if (DMiscUtil.isFullParticipant(p) && DMiscUtil.hasDeity(p, getName())) {
            ...
            if (((Oceanus) DMiscUtil.getDeity(p, getName())).SKILLBIND != null)
                p.sendMessage(ChatColor.AQUA + "    Bound to " + ((Oceanus) DMiscUtil.getDeity(p, getName())).SKILLBIND.name());
    ...
    public void onTick(long timeSent) {
        ...
        if (timeSent > LASTCHECK + (healinterval * 1000)) {
            ...
            if ((DMiscUtil.getOnlinePlayer(getPlayerName()) != null) && DMiscUtil.getOnlinePlayer(getPlayerName()).getWorld().hasStorm()) {
                ...
                if (DMiscUtil.getHP(getPlayerName()) + healamt > DMiscUtil.getMaxHP(getPlayerName()))
                    healamt = DMiscUtil.getMaxHP(getPlayerName()) - DMiscUtil.getHP(getPlayerName());

...
public class Rhea implements Deity {
    ...
    private final int PLANTCOST = 110;
    private final int RHEAULTIMATECOST = 700;
    ...
    public void printInfo(Player p) {
        if (DUtil.hasDeity(p, "Rhea") && DUtil.isFullParticipant(p)) {
			 ...
			 */
            //poison
            int duration = (int) Math.floor(4.95778 * Math.pow(DUtil.getLevel(p), 0.4590192)); //seconds
            if (duration < 1) duration = 1;
            int strength = (int) (Math.floor(1.9115168 * Math.pow(DUtil.getLevel(p), 0.32108423))); //1-6
            //explosion
            float explosionsize = (float) (0.0867 * DUtil.getLevel(p) + 2.91816);
            //ultimate
            int range = (int) (10.84198 * Math.pow(1.01926, DUtil.getLevel(p)));
            int ultimateduration = (int) (4.95778 * Math.pow(DUtil.getLevel(p), 0.459019));
			 ...
			 * The printed text
			 */
            p.sendMessage("--" + ChatColor.GOLD + "Rhea" + ChatColor.GRAY + "[" + DUtil.getLevel(p) + "]");
            ...
            p.sendMessage("Explosion radius: " + explosionsize + ". Maximum trees: " + DUtil.getLevel(p));
        ...
        p.sendMessage(ChatColor.YELLOW + "Select item: log");
    }
    ...
    private boolean poison(Player p) {
        ...
        if (target != null) {
            ...
            final PotionEffect poison = new PotionEffect(PotionEffectType.POISON, duration, strength);
            pt.addPotionEffect(poison);
            ...
            DUtil.getPlugin().getServer().getScheduler().scheduleAsyncDelayedTask(DUtil.getPlugin(), new Runnable() {
                @Override
                public void run() {
                    pt.removePotionEffect(PotionEffectType.POISON);
                }
            }, duration * 20);
            p.sendMessage(ChatColor.YELLOW + pt.getName() + " has been poisoned for " + duration + " seconds.");
            pt.sendMessage(ChatColor.RED + "You have been poisoned for " + duration + " seconds.");
            return true;
        } else {
    ...
    private boolean plant(Player player) {
        ...
        if ((b != null) && !DUtil.isProtected(b.getLocation())) {
            if (TREES.size() == DUtil.getLevel(player)) {
                player.sendMessage(ChatColor.YELLOW + "You may place a maximum of " + TREES.size() + " trees.");
                return false;
            }
    ...
    private void detonate(Player player) {
        float explosionsize = (float) (0.0867 * DUtil.getLevel(player) + 2.91816);
        if (TREES.size() > 0) {
            for (WriteLocation w : TREES) {
                ...
                if (l.getBlock().getType() == Material.LOG) {
                    ...
                    for (Entity e : l.getBlock().getChunk().getEntities()) {
                        if (e instanceof LivingEntity) {
                            LivingEntity le = (LivingEntity) e;
                            if (le.getLocation().distance(l) < explosionsize)
                                le.damage((int) (explosionsize - le.getLocation().distance(l)), player);
                        }
                    }
                }
            }
    ...
    private int entangle(Player p) {
        int range = (int) (10.84198 * Math.pow(1.01926, DUtil.getLevel(p)));
        int duration = (int) (4.95778 * Math.pow(DUtil.getLevel(p), 0.459019));
        int count = 0;
